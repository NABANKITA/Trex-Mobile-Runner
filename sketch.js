...
function setup() {
  ...
  // cloud.velocityX=-4;
  road = createSprite(1000, height/2+80,width, windowHeight);
  road.addAnimation("a",p2);
  //dinosaur.scale=0.8;
  road.velocityX=-6;
  road_2 = createSprite(200,height/2+94,windowWidth, windowHeight-500 );
  //road_2.shapeColor=250;
  road_2.visible=false
  ob_group=new Group();
  cl_group=new Group();
    gameOver = createSprite(width/2, windowHeight-400);
   gameOver.addAnimation("c",p11);
     gameOver.scale=0.8;
    restart = createSprite(width/2, windowHeight-350);
...
function draw() {
 ...
 text("score:"+score_a()+Math.round(score),windowWidth-100, 40);
  drawSprites();
